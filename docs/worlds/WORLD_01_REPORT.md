...
| **Projects Section** | `components/biyahe/BiyaheProjects.tsx` | Chalk band featuring 4 full-width stacked destination boards (`STOP 02-A` through `STOP 02-D`) offering a comprehensive engineering view. Provides immediate, unmissable access to live applications (`[★ OPEN LIVE APP ↗]`) and GitHub repositories (`[GITHUB REPO ↗]`) for ReadHub and Gordon RamsAi, and `[★ GITHUB REPO ↗]` for DaloyAqua. For PlayIT, displays a prestigious stamped gold emblem (`★ FLAGSHIP CAPSTONE THESIS • 100% ON-DEVICE OFFLINE`). Every board features a comprehensive description, 3-column field problem/constraints/architecture grid, key deliverables, stack tags, and an inline `[▼ EXPAND 4-STAGE ARCHITECTURE & DATA CONTRACTS]` toggle revealing verified KPI badges, full data contracts (`[IN]`, `[PROC]`, `[OUT]`), and engineering tradeoffs. |
| **Skills Section** | `components/biyahe/BiyaheSkills.tsx` | Leaf band displaying all 5 category module strips directly: cycling enamel end-cap plates (Sun, Signal, Cobalt, Chalk) with route codes, skill counts, and focus descriptions, paired with full grids of tactile Chalk chips (Lexend 600, 1rem, 3px black outline, 8px radius, 3px depth offset). |
...
| **Contact Section** | `components/biyahe/BiyaheContact.tsx` | Signal band featuring a large Sun mailto board (`mailto:rivazendrix@gmail.com`), Copy Address pressable plate with live-region announcement, external terminal plates (GitHub, LinkedIn), grounded black chassis footer, and a stamped Cobalt `[▲ BACK TO FIRST STOP]` smooth scroll return plate. |
...
3. **Middle Sections Presentation Polish:** Refactored Journey (`BiyaheExperience`) to a 3-row desktop highway architecture guaranteeing 100% horizontal road alignment across variable-height cards; enhanced About (`BiyaheAbout`) with vehicle decal street code callouts and rivet accents; and eliminated murky background opacity in Skills (`BiyaheSkills`) using crisp chalk chassis boards.
4. **Deep Engineering Project Architecture & Data Contracts:** Extracted and exposed complete 4-stage architecture pipelines (`01` through `04`), data contracts, specs, and engineering tradeoffs directly from `data/projects.ts`.
5. **Comprehensive Full-Width Destination Boards & Immediate Link Access:**
   - **Comprehensive View:** Replaced cramped cards with 4 full-width stacked transit destination boards (`STOP 02-A` through `STOP 02-D`), giving reviewers a complete, immersive view of each project directly while scrolling down the page.
   - **Immediate & Prominent Link Access:** Direct, full-sized action buttons are permanently visible at the top and bottom of each board:
     - **ReadHub:** `[★ OPEN LIVE APP ↗]` (Signal red) &rarr; `https://readhub-three.vercel.app/login` + `[GITHUB REPO ↗]` (Cobalt).
     - **Gordon RamsAi:** `[★ OPEN LIVE CHATBOT ↗]` (Signal red) &rarr; `https://gordon-ramsai.streamlit.app/` + `[GITHUB REPO ↗]` (Cobalt).
     - **DaloyAqua:** `[★ GITHUB REPO ↗]` (Cobalt) &rarr; `https://github.com/zendrix-hub/daloyaqua` + `[BACKEND IN PROGRESS]` badge.
...
6. **Footer Chassis Grounding & Return Plate:** Fixed the layout where the red Contact section bottom padding was leaking beneath the black footer. Extracted the black footer (`<footer>`) into a dedicated bottom landmark separated by `BiyahePinstripe`, anchoring it solidly to the very bottom of the document with zero color bleeding. Added a stamped Cobalt pressable plate `[▲ BACK TO FIRST STOP]` (`#1B3FD1`, white text, 3D depth band) to allow visitors to smoothly scroll back to the beginning of the route.
