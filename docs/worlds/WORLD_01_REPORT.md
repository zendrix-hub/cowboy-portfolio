...
| **Projects Section** | `components/biyahe/BiyaheProjects.tsx` | Chalk band featuring a balanced 2x2 grid of dedicated terminal cards (`STOP 02-A` through `STOP 02-D`). Each card displays the project title, subtitle, engineering role context, description, field problem & constraints plate, 4-stage architecture pipeline breadcrumb, key deliverables, system stack tags, and prominent direct action buttons (`[★ OPEN LIVE APP ↗]` and `[GITHUB REPO ↗]` for ReadHub and Gordon RamsAi, `[GITHUB REPO ↗]` for DaloyAqua, and a stamped `[OFFLINE CAPSTONE THESIS]` badge for PlayIT without external links). Snug vertical padding eliminates dead space before Skills. |
...
5. **2x2 Project Terminal Grid & Snug Spacing Architecture:**
   - **Clean 2x2 Layout:** Replaced the single-featured accordion layout with a clean, balanced 2x2 grid of dedicated terminal cards (`STOP 02-A` through `STOP 02-D`), giving every project first-class visibility without needing to toggle tabs or open disclosure accordions.
   - **Prominent Direct Action Buttons:** Added large, unmissable `[★ OPEN LIVE APP ↗]` (Signal red) and `[GITHUB REPO ↗]` (Cobalt) buttons pinned directly to the bottom of cards for ReadHub, Gordon RamsAi, and DaloyAqua.
