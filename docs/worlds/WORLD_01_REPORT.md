...
| **Projects Section** | `components/biyahe/BiyaheProjects.tsx` | Chalk band featuring a balanced 2x2 grid of dedicated terminal cards (`STOP 02-A` through `STOP 02-D`) designed for rapid 10-second recruiter scans (1-sentence hook, role context, 3 crisp engineering outcome bullet points, stack chips, direct live/repo links, and an `[INSPECT FULL SPECS ➔]` button). Clicking Full Specs opens a high-contrast Centered Transit Blueprint Modal with complete problem/constraints, verified KPI badges, and the full 4-stage architecture pipeline with data contracts (`[IN]`, `[PROC]`, `[OUT]`). |
...
5. **2x2 Project Terminal Grid & Transit Blueprint Modal Architecture:**
   - **Rapid Recruiter Scanability:** Streamlined the card front to eliminate text bloat and walls of text. Each card now presents a punchy 1-sentence hook, engineering role context, 3 concrete outcome bullet points, stack chips, direct live/repo action buttons, and a stamped `[FULL SPECS ➔]` button.
   - **Centered Transit Blueprint Modal:** Clicking `FULL SPECS` triggers a high-contrast, accessible modal dialog (`role="dialog"`, Esc to dismiss, body scroll locked) styled as an authentic vehicle engineering manifest. It holds 100% of the repository's deep technical documentation: verified LTO inspection KPI scorecards, field problem & constraints breakdown, complete 4-stage architecture pipeline with data contracts (`[IN]`, `[PROC]`, `[OUT]`), specs, and engineering tradeoffs.
   - **Prominent Direct Action Buttons:** Added large, unmissable `[★ OPEN LIVE APP ↗]` (Signal red) and `[GITHUB REPO ↗]` (Cobalt) buttons pinned directly to the bottom of cards for ReadHub and Gordon RamsAi, and `[GITHUB REPO ↗]` for DaloyAqua.
   - **PlayIT Code Restriction:** Sealed PlayIT with an authentic `[OFFLINE CAPSTONE THESIS • 100% ON-DEVICE]` plate with no external links, preserving its offline thesis status per owner instructions.
   - **Snug Vertical Spacing:** Capped vertical padding across Projects (`pb-8 sm:pb-10 lg:pb-12`) and Skills (`pt-8 sm:pt-10 lg:pt-12`), eliminating the previous ~200px void for a seamless transit body panel flow across `BiyahePinstripe`.
