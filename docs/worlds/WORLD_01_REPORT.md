...
| **Projects Section** | `components/biyahe/BiyaheProjects.tsx` | Chalk band featuring an interactive flagship dispatch selector (`DaloyAqua`, `PlayIT`, `ReadHub`, `Gordon RamsAi`), full-width Cobalt board with role badge, engine/capacity spec plates, dynamic SVG telemetry schematic, problem/solution/constraints breakdown, complete 4-stage architecture pipeline with explicit data contracts (`[IN]`, `[PROC]`, `[OUT]`), performance benchmarks, and engineering tradeoffs. Supporting projects render as expandable Route List rows (`<details>`) with isolated CTA plates and comprehensive architecture disclosures. |
...
4. **Deep Engineering Project Architecture & Data Contracts:** Completely overhauled `BiyaheProjects.tsx` to surface the full depth of `data/projects.ts`. Rather than basic cards, the section now presents the complete 4-stage architecture pipeline (`ArchitecturePipelineFlow`) for every project with explicit data contracts (`[IN]`, `[PROC]`, `[OUT]`), performance thresholds (e.g. `200ms processing threshold`, `4GB RAM target`), architectural tradeoffs, constraints, deliverables, and role context.
5. **Interactive Showcase Enhancements & Transit Link Architecture:**
   - **Flagship Focus:** `PlayIT` designated as default flagship board, showcasing its on-device phonics thesis architecture.
   - **Link Protection:** As requested, `PlayIT` is sealed with an authentic `[OFFLINE CAPSTONE THESIS • INSTITUTIONAL REPOSITORY]` badge with no external links, while `ReadHub` (`liveUrl` + GitHub), `Gordon RamsAi` (`liveUrl` + GitHub), and `DaloyAqua` (GitHub) feature prominent external plates directly on both the outer route list rows (with `e.stopPropagation()`) and expanded cards.
   - **Verified LTO Inspection KPI Badges:** Added stamped 4-card metric scorecards for each project (e.g. `0 KB Cloud Egress`, `<40ms Audio Latency`, `>500 req/s Ingestion`, `<85ms Local Vector RAG`).
   - **Interactive Pipeline Simulation:** Added auto-simulation and click-to-inspect stepping across the 4 architecture pipeline stages, synchronized with the real-time telemetry monitor in `DynamicSchematicWindow`.
